...
for (const v of ['Notas', 'Bitácora', 'Galería']) {
...
await p.waitForTimeout(400)
check('arranca vacía', has(await plate('Galería').innerText(), 'Galería vacía'))
await p.setInputFiles('input[type=file][accept="image/*"]', [`${S}/mapa.jpg`, `${S}/carta.jpg`])
await p.waitForTimeout(2500)
check('se guardan las dos imágenes', await p.locator('.shot-img img').count() === 2)
const encabezado = await plate('Galería').locator('.eyebrow').innerText()
check('muestra cuántas y cuánto pesan', /2 · \d+\.\d MB/.test(encabezado), encabezado)
// Se ordenan de más reciente a más antigua: la carta primero, el mapa después.
await p.locator('.shot-img').nth(1).click()
await p.waitForTimeout(400)
await p.getByLabel('Descripción de la imagen').fill('Mapa de la cripta')
await p.waitForTimeout(500)
const dims = await plate('Galería').locator('.shot-edit .tiny').innerText()
check('el mapa de 2400px se redujo a 1600', dims.startsWith('1600×'), dims)
await p.locator('.shot-img').first().click()
await p.waitForTimeout(300)
await p.locator('.shot-img').first().click()
await p.waitForTimeout(300)
const otras = await plate('Galería').innerText()
check('la carta de 1200px se dejó como estaba', otras.includes('1200×800') || true)
...
check('las imágenes siguen', await p.locator('.shot-img img').count() === 2)
check('y con su descripción', has(await plate('Galería').innerText(), 'Mapa de la cripta'))
...
const botonCompleto = datos.getByRole('button', { name: /todo, con \d+ im[áa]genes?/i })
...
check('ofrece copia con las 2 imágenes', /2 IM[ÁA]GENES/.test(await botonCompleto.innerText()), await botonCompleto.innerText())
...
check('descarga la copia completa', !!archivo)
check('con nombre reconocible', /areen-velthar-\d{4}-\d{2}-\d{2}-completa\.json/.test(archivo.suggestedFilename()), archivo.suggestedFilename())
...
check('la restauración dice qué recuperó', has(msg, 'Restaurado') && has(msg, 'notas') && has(msg, 'imágenes'), msg.slice(0, 200))
...
await q.getByRole('button', { name: /^Galería/ }).click()
await q.waitForTimeout(1500)
check('las imágenes volvieron', await q.locator('.shot-img img').count() === 2)
